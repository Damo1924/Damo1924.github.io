...
Multimap과 multiset은 중복 원소를 허용한다는 점을 제외하고는 map, set과 완전히 동일한 컨테이너이다. 다만, 중복 원소를 허용하기 때문에 원소에 접근하는 방식이 조금 다르다. 기존 set이나 map에서 사용하던 접근 방식들은 다음과 같은 문제가 있다.
...
그렇다면 특정 key에 여러 개의 데이터가 저장되어 있을 때는 어떻게 접근해야할까? 
