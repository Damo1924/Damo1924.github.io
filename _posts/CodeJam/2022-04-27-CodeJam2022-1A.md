...
$l < r$인 두 정수 $l, r$에 대하여 $l$번째 운동부터 $r - 1$번째 운동까지 공통으로 사용하는 원판들의 multiset을 $C(l, r)$,

$C(l,r)$에 속하는 원판이 기구에 기본적으로 걸려 있는 상황에서 $l$번째 운동부터 $r-1$번째 운동까지 진행하는 동안 원판을 옮기는 횟수의 최솟값을 $M(l, r)$이라고 정의하자.

모든 $(l, r)$ 정수 쌍에 대해 $C(l, r)$에 몇 개의 원판이 있는지 구하는데 $O(E^2 \times W)$의 시간복잡도가 소요된다.

$C(l, r)$에 있는 원판의 개수를 $\left\vert C(l, r) \right\vert$라고 하면, $M(l, r)$은 다음과 같이 구할 수 있다.

$l \leq x < r$을 만족하는 정수 $x$에 대해 $M(l, r)$을 $M(l, x)$와 $M(x, r)$으로 표현해보자.

$M(l, r)$에는 기본적으로 기구에 $C(l, r)$에 속하는 원판들이 이미 기구에 있으므로,

$x-1$번째 운동을 하기 전과 후에 각각 $C(l, x) - C(l, r)$를 기구에 추가하고 제거하고,

$x + 1$번째 운동을 하기 전과 후에 각각 $C(x, r) - C(l, r)$을 기구에 추가하고 제거해야한다.

따라서 다음이 성립한다.

\begin{aligned}
M(l,r) = \min_{l \leq x < r} \left( M(l, x) + M(x, r) + 2 \times ( \left\vert C(l, x) \right\vert + \left\vert C(x, r) \right\vert - 2 \times \left\vert C(l, r) \right\vert ) \right)
\end{aligned}

모든 $(l, r)$ 정수 쌍에 대해 $M(l, r)$을 구하는데 $O(E^3)$의 시간복잡도가 소요된다.

따라서 전체 시간복잡도는 $O(E^2(E + W)$이고, 이는 시간 내에 통과하기에 충분하다.
