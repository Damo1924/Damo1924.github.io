...
- [Saving Changes](#saving-changes): `git add`, `git commit`, `git diff`, `git stash`, `.gitignore`
...
- **git status**
  - `$ git status` : Commit해야하는 모든 파일을 나열
- **git tag**
  - `$ git tag` : 현재 repository에 있는 모든 tag를 나열
  - `$ git tag [TagName]` : 현재 버전에 tag를 생성
- **git blame**
  -
...
- **git revert**
- **git reset**
  - `$ git reset --hard` : 마지막 commit 이후의 모든 변경 사항을 취소(undo)
  - `$ git reset --hard ORIG_HEAD` : 가장 최근에 실행된 merge와 그 이후의 변경 사항들을 취소
  - `$ git reset --soft HEAD^` : 가장 최근 commit을 취소, 그러나 변경 사항들은 stage된 상태로 유지
- **git rm**
