...
[BOJ 18227. 성대나라의 물탱크 문제 링크](https://www.acmicpc.net/problem/18227)

$1$번부터 $N$번까지 번호가 매겨진 도시들이 트리의 형태로 연결되어 있다. (트리의 루트가 수도이다.)

각 도시의 물탱크는 초기에는 전부 비어있으며, 다음과 같은 방법으로 물탱크를 채우려고 한다.

- $A$번 도시에 물을 채우기로 했다면, 수도에서부터 $A$번 도시까지 잇는 직선 경로에 수도부터 차례대로 $1$L, $2$L, ... 만큼의 물이 추가된다.
- $A$번 도시에는 수도에서 $A$번 도시까지의 도시 수만큼 물이 추가되는 것이다.

$A$번 도시에 물을 채우는 쿼리와 $A$번 도시에 물이 얼마나 있는지를 묻는 쿼리가 총 $Q$개 주어진다.

**[SOLUTION]**

어떤 도시에 물이 얼마나 있는지를 구하기 위해서는 해당 도시를 루트로 하는 서브트리에 포함되는 도시들에 물이 추가된 횟수를 알아야한다.

따라서 어떤 도시에 물을 추가한 횟수를 저장해두면서, 동시에 해당 도시를 루트로 하는 서브트리에 포함되는 도시들에 물이 추가된 횟수를 구할 수 있는 세그먼트 트리를 만들면 된다.

물을 채우는 쿼리는 특정 노드의 값을 $1$만큼 증가시키는 쿼리로, 물의 양을 구하는 쿼리는 구간에 포함되는 노드들의 합을 구하는 쿼리로 생각할 수 있다.

이때 특정 도시에 추가되는 물의 양은 항상 **해당 도시의 깊이**(수도의 깊이를 $1$이라 하자)와 같다.

그러므로 어떤 도시에 있는 물의 양은 (해당 도시에 물이 추가된 횟수)와 (해당 도시의 깊이)를 곱한 것과 같다.

---

### [BOJ] 18437. 회사 문화 5

[BOJ 18437. 회사 문화 5 문제 링크](https://www.acmicpc.net/problem/18437)

$1$번부터 $N$번까지 번호가 매겨진 직원들이 재직 중인 회사가 있고, 대표는 $1$번 직원이다.

대표를 제외한 직원들은 한 명의 직속 상사를 가지기 때문에 직원들의 관계를 하나의 트리로 생각할 수 있다.

초기에 모든 직원의 컴퓨터가 켜져 있을 때, 다음과 같은 쿼리를 처리하고자 한다.

1. $i$번 직원의 부하직원들은 모두 컴퓨터를 켠다.
2. $i$번 직원의 부하직원들은 모두 컴퓨터를 끈다.
3. $i$번 직원의 부하직원들 중 컴퓨터가 켜져 있는 사람의 수를 출력한다.

**[SOLUTION]**

구간 업데이트와 구간 쿼리를 처리해야하는 문제이기 때문에 lazy propagation을 구현한 세그먼트 트리를 이용해야한다.

이 외에도 회사 문화 2, 3, 4는 ETT로 해결할 수 있는 간단한 문제들이다.
