...
[BOJ 13546. 수열과 쿼리 4 문제 링크](https://www.acmicpc.net/problem/13546)

주어진 구간에서 $a_i = a_j$ 를 만족하는 $i, j$ 에 대하여 $\left\vert i - j \right\vert$ 의 최댓값을 구하는 문제이다.

구간의 왼쪽 끝이 동일한 블록에 속하는 것들끼리 묶어준 후, 묶은 것들을 구간의 오른쪽 끝에 대한 오름차순으로 정렬했다고 하자.

현재 블록의 범위를 $\[lb, rb\]$, 해당 블록에 구간의 왼쪽 끝이 포함되는 쿼리 $s, e$($lb \leq s \leq rb$) 이 주어졌을 때

1. 두 인덱스 $i, j$가 구간 $(rb, e\]$ 에 포함될 때의 최댓값
2. 인덱스 $i$가 구간 $\[s, \min(rb, e)\]$ 에 포함될 때의 최댓값

을 구한다면, 원하는 답을 구할 수 있게 된다.

이때 **최댓값은 어떤 정수 $k$가 등장하는 가장 큰 인덱스와 가장 작은 인덱스의 차이**로 구할 수 있다.

해당 블록에 구간의 왼쪽 끝이 포함되는 쿼리들을 정렬해두었으므로 이 쿼리들 전부에 대해 1번을 $O(n)$으로 구할 수 있다.

2번에서 가능한 $i$는 $O(\sqrt{n})$개이므로 직접 하나하나 구해주어도 된다.

따라서 해당 블록에 왼쪽 끝이 포함되는 쿼리들의 개수를 $q$라고 하면, 이 쿼리들을 처리하는데 $O(n + q \sqrt{n})$이 걸리게 된다.

전체 블록의 개수가 $O(\sqrt{n})$ 이고, 쿼리의 총 개수 $Q = \sum q$ 이므로 전체 시간복잡도는 $O((n + Q) \sqrt{n})$ 이다.

---

### [BOJ] 13550. 수열과 쿼리 7

[BOJ 13550. 수열과 쿼리 7 문제 링크](https://www.acmicpc.net/problem/13550)

주어진 구간에 포함되는 부분수열이면서, 원소들의 합을 어떤 정수 $K$로 나눈 값이 $0$이 되는 것들 중 길이가 가장 긴 것을 구하는 문제이다.

원소들의 합을 어떤 정수 $K$로 나눈 값이 $0$이 되는 부분수열은, **prefix sum을 구했을 때 같은 값을 가지는 두 인덱스**를 통해 구할 수 있다.

이때 우리에게 필요한 것은 위 조건을 만족하는 부분수열 중 가장 긴 것의 길이이므로, 바로 전 문제와 동일하게 **prefix sum이 $S$인 가장 큰 인덱스와 가장 작은 인덱스**를 저장해둠으로써 구할 수 있다.
