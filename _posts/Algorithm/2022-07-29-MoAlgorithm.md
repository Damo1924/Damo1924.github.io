...
평면 분할에서 했던 것처럼 전체 구간을 $\sqrt{n}$개의 크기의 블록들로 나눈 후, 주어진 쿼리 $\[l_j, r_j\]$ 들을 아래 규칙으로 정렬하자.

1. $r_j$이 속한 블록이 증가하는 순서대로 정렬
2. 만약 $r_j$이 속한 블록이 같다면, $l_j$이 증가하는 순서대로 정렬

이제 $l, r$ 을 움직여가면서 위 순서대로 쿼리를 처리해줄 것이다.

구간의 오른쪽 끝을 나타내는 $r$은 쿼리마다

- 하나의 블록 내에서 최대 $\sqrt{n}$
- 다른 블록으로 이동할 때 
