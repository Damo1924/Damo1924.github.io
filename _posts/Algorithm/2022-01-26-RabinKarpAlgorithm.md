...
알파벳 소문자로만 이루어진 길이 $L(1 \leq L \leq 200000)$인 문자열이 주어진다.

이 문자열의 부분문자열 중, 적어도 한 번은 다시 나타나는 부분문자열을 '반복 부분문자열'이라고 부르자.

주어진 문자열의 가장 긴 반복 부분문자열의 길이를 구하는 문제이다.

만약 반복 부분문자열이 하나도 존재하지 않으면 0을 출력한다.

**[Solution]**

이 문제의 핵심은 **반복 부분문자열의 길이에 대해 해당 문자열이 나타나는 횟수가 감소한다**는 것이다.

**이분탐색**을 이용하면 가장 긴 반복 부분문자열을 찾을 수 있게 된다.
...
이 외에도 Reference [2]에 라빈-카프 알고리즘으로 해결할 수 있는 재미있는 문제들이 많이 소개되어 있으니 더 많은 문제를 풀고 싶다면 참고!
...
[2] [BAEKJOON, rdd6584, '해시로 장난치기'](https://www.acmicpc.net/blog/view/67)
