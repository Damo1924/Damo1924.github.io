...
다익스트라 알고리즘의 구현은 가중치가 없는 그래프에서 BFS로 최단 경로를 찾는 것과 그 형태가 유사하다.
...
꼭짓점의 개수를 $\left\vert V \right\vert$라고 할 때, $O(\left\vert V \right\vert^2)$의 시간복잡도로 어떤 한 꼭짓점으로부터 다른 모든 꼭짓점까지의 최단 경로를 구할 수 있다.

하지만, 이후 큐 대신 **우선순위 큐**를 사용하는 알고리즘이 등장하게 되면서 $O(\left\vert E \right\vert + \left\vert V \right\vert \log \left\vert V \right\vert)의 시간복잡도를 가지게 되었다.
