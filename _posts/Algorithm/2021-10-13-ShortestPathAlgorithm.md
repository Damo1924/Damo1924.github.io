...
음의 사이클이란 그래프에서 **가중치의 총합이 음수가 되는 사이클**을 말하는데, 다익스트라는 음의 사이클을 만나면 사이클 위의 정점들까지의 거리를 계속해서 더 작은 값으로 갱신하게 되어 사용할 수 없다.

벨만-포드 알고리즘은 **음의 사이클이 발생해서 최단거리를 구할 수 없다**는 것을 확인할 수 있는 알고리즘으로, 다음과 같은 과정을 통해 이루어진다.

---

1. 모든 간선을 순회하면서 각 간선을 통해 특정 정점에 도달할 수 있는 최단 경로를 갱신한다.
2. 그래프의 정점의 개수를 V라고 하면, 위 과정을 (V-1)번 반복한다.
3. 1번 과정을 한 번 더 반복하면서 음의 사이클이 존재하는지 확인한다.

---

모든 최단경로는 최대 (V-1)개의 간선으로 이루어지기 때문에 모든 간선을 탐색하는 과정을 (V-1)번 반복하면 반드시 모든 최단경로를 구할 수 있다.

그러므로 음의 사이클이 없는 그래프라면 V번째 탐색에서 어느 최단경로도 갱신되지 않는다.

즉, V번째 탐색에서 최단경로가 갱신된다면 음의 사이클이 존재한다는 사실을 알 수 있는 것이다.

비록 다익스트라에 비해 시간복잡도 면에서는 좋지 못해도 음의 가중치를 가진 그래프에서도 최단경로를 찾을 수 있다는 장점이 있다.

모든 간선을 순회하는 과정을 정점의 개수만큼 반복하므로 벨만-포드 알고리즘의 시간복잡도는 $O(\left\vert V \right\vert \left\vert E \right\vert)$가 된다.
...
N개의 도시가 있고, 한 도시에서 출발하여 다른 도시에 도착하는 버스가 M개 있다.

각 버스는 시작도시, 도착도시, 버스를 타고 이동하는데 걸리는 시간으로 이루어져있고, 이때 시간은 양수가 아닐 수도 있다.

1번 도시에서 출발해서 나머지 도시로 가는 가장 빠른 시간을 구하고, 만약 해당 도시로 가는 경로가 없다면 -1을 출력한다.
