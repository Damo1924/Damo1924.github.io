...
위 과정은 **Push Up**(또는 **Bubble-Up**)이라고 불리는 알고리즘으로 구현된다.
...
void pushUp_min(int idx) // min level을 따라 루트로 올라가는 함수
{
    int grandparent = idx / 2 / 2;
    if (grandparent == 0) return;
    if (MMH[idx] < MMH[grandparent])
    {
...
void pushUp_max(int idx) // max level을 따라 루트로 올라가는 함수
{
    int grandparent = idx / 2 / 2;
    if (grandparent == 0) return;
    if (MMH[idx] > MMH[grandparent])
...
void insert(int n)
{
    ...
    int idx = MMH.size() - 1, h = floor(log2(idx)); // idx: 삽입된 원소의 인덱스, h: 삽입된 원소의 레벨
    if (idx == 1) return; 
    
    if (h % 2) // idx: max level
    {
        if (MMH[idx] < MMH[idx / 2]) // 부모 노드가 더 크면,
        {
            swap(MMH[idx], MMH[idx / 2]);
            pushUp_min(idx / 2); // min level 노드들과 비교
        }
        else pushUp_max(idx);
    }
    ...
    {
        if (MMH[idx] > MMH[idx / 2]) // 부모 노드가 더 작으면,
        {
            swap(MMH[idx] > MMH[idx / 2]);
            pushUp_max(idx / 2); // max level 노드들과 비교
        }
        else pushUp_min(idx);
    }
}
...
원소를 삭제하는 과정도 삽입과 비슷하게 진행된다.

> 1) 삭제하려는 원소가 min level
