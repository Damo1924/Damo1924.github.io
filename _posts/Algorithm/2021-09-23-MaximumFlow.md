...
그리고 Edmond-Karp Algorithm의 시간복잡도를 증명할 때와 마찬가지로 한 최단 경로를 지날 수 있는 유량을 탐색해주고 나면, 적어도 하나의 간선은 용량을 꽉 채우게 되고, 해당 간선을 지나는 최단 경로는 존재하지 않는다는 사실을 이용하면 차단 유량을 구하는데 과정의 시간복잡도는 $O(VE)$이다.
...
**Lemma 1.** 소스와 싱크 사이의 최단 거리가 $E^{1/2}$ 이상인 네트워크에서 $E^{1/2}$ 이하의 크기를 갖는 Cut을 찾을 수 있다.
...
**Lemma 2.** 소스와 싱크 사이의 최단 거리가 $2V^{2/3}$ 이상인 네트워크에서 $V^{2/3}$ 이하의 크기를 갖는 Cut을 찾을 수 있다.
...
Max-Flow Min-Cut Theorem에 의해 Max-Flow의 크기와 Min-Cut의 크기는 동일하므로 
