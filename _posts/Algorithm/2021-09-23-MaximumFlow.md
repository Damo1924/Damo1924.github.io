...
> 동일한 정점을 다시 방문하지 않는 조건에서의 최대 유량은 **정점 분리**를 통해 구할 수 있다.
> 
> - 정점 $i$로 들어오는 간선과 연결된 정점 $i_{in}$
> - 정점 $i$에서 나가는 간선과 연결된 정점 $i_{out}$
> 
> $i_{in}$에서 $i_{out}$으로 향하는 최대 용량이 $1$인 간선을 추가해주면 각 정점을 최대 한 번만 방문하게 만들 수 있다.

**무향 그래프**의 최대 유량 문제를 풀 때는 양쪽 방향 모두 같은 최대 용량을 설정한다.

> MCMF에서는 역방향 간선의 비용이 정방향 간선의 비용에 $-1$을 곱한 값이기 때문에 무향 그래프를 이처럼 처리할 수 없지만, 최대 유량 문제에서는 괜찮다.
...
<img src="https://user-images.githubusercontent.com/88201512/134772866-340e8b4f-a538-42cf-9f52-84a14d763333.jpg" width="70%" height="70%">
