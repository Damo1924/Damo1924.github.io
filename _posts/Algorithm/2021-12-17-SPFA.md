...
> 
> 반대로 relaxation이 더 가능하다면, 큐가 비어있지 않으므로 알고리즘이 종료되지 않는다.
 ...
 **[Lemma 2]** 음의 사이클(negative-weight cycle)이 존재한다면, 알고리즘은 종료되지 않고 무한히 반복된다.
 ...
 > 
 > $\therefore$ 음의 사이클이 있으면 반드시 relaxation이 가능한 간선이 존재한다. (= 알고리즘이 종료되지 않고 무한히 반복된다.)

---

**[Theorem]** 더 이상 relaxatioin이 불가능하다면 알고리즘으로 구해진 $s$로부터 다른 정점들까지의 거리들은 모두 정확한 값이다. 반대로 어떤 정점 $u$에 대해 $s$부터 $u$까지의 거리가 정확하지 않다면 relaxation이 가능한 간선이 그래프에 반드시 존재한다.

> ***Proof.***
> 
> 소스 $s$에서 정점 $u$까지의 최단 경로에 있는 간선들의 개수에 대한 귀납법을 이용할 것이다.
> 1
> 먼저 최단 경로에 있는 간선이 하나라면 정점 $u$에 대한 첫 relaxation에서 최단 경로를 바로 얻을 수 있으므로 
