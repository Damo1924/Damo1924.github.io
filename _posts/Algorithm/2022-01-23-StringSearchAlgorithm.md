...
그러나 KMP 알고리즘은 지금까지 탐색한 패턴(ABCAB)의 앞과 뒤에 겹치는 부분(AB)가 있다는 정보를 토대로, 중간 단계를 건너뛰고 AB로 시작하는 텍스트와 패턴을 비교하게 된다.

이때 텍스트와 패턴의 첫 두 글자가 AB로 동일하다는 사실을 알고 있으므로 패턴의 세 번째 글자부터 탐색하면 된다.
...
그렇다고 텍스트의 두 번째 글자(B)부터 시작하는 부분(BCA), 세 번째 글자 (C)부터 시작하는 부분(CAB)과 패턴을 비교할 필요는 없다.
...
> `pi[j]` = 패턴에서 인덱스 0~j에 해당하는 부분에 대해 다음 조건을 만족하는 접두사 중 가장 긴 것의 길이
> 
> - **접두사와 접미사가 동일**
...
**2.** `i`를 1씩 증가시키면서 다음 과정을 반복한다.
  ...
  **2-2.** `text[i] != pattern[j]`이면 `j = pi[j - 1]`
