...
- $dp(m, y)$: $m$번째 bit까지 채웠을 때 ($m$번째 자리까지 채웠을 때 수들의 합)$=y$ 인 경우의 수

위와 같은 DP 배열을 정의해야하는데, 입력으로 주어질 수 있는 상한 $x$의 범위가 $10^{18}$ 이기 때문이다.

이와 같은 문제점을 해결해주기 위해서 $y=$($m$번째 자리까지 채웠을 때의 합)을 기억하는 대신, $x - y$에 $m$번째 bit가 몇 개나 더 들어갈 수 있는지를 기억해준다.

아래와 같은 DP 배열을 정의하자.

- $dp(m, k)$: $k=\min($ ($x - y$에 $m$번째 bit가 들어갈 수 있는 개수), $2n)$ 이도록 첫 $m$개의 bit를 채우는 방법의 수

그렇다면 아래 식이 성립한다.

\begin{aligned}
dp(m, k) = \sum_{j = z_m}^{\min(n, k)} {n \choose j} \cdot dp(m + 1, 2(k - j + x_m)
\end{aligned}

이때 $j$, $z_m$, $x_m$은 다음을 의미한다.

- $j$: 수열에 있는 $n$개의 수 중에서 $m+1$번째 bit를 가지는 수의 개수
- $z_m$: 주어진 $z$가 $m$번째 bit를 가지면 $1$, 아니면 $0$ (두 번째 조건을 만족하기 위함)
- $x_m$: 수열의 합이 $x$를 넘지 않도록 
...
ll l, r, z;
int n, comb[1001][1001], dp[60][2001];
...
int g(int i, int j)
{
    ...
    for (int k = ((z & (1ll << i)) ? 1 : 0); k <= min(n, j); k += 2)
        res = (res + 1ll * pCq(n, k) * g(i - 1, 2 * (j - k))) % MOD;
    return res;
}
