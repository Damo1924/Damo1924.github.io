...
- Color $l$ $r$ $c$ ($l = r = i$, $c' = color\[i\]$): a\[i\] := a\[i\] + lazy\[c'\] - lazy\[c\], color\[i\] := c
- Add $c$ $x$: $lazy\[c\] := lazy\[c\] + x$
- Query $i$: print $a\[i\] + lazy\[color\[i\]\]$
...
구간 업데이트를 빠르게 수행할 수 있는 **세그먼트 트리** 또는 **펜윅 트리**를 이용하면 각 쿼리를 $O(\log n)$에 처리할 수 있다.
