...
교점의 위치가 $\frac{S+1}{3}$인 것과 $cnt_1$이 적어도 높이 차가 홀수인 나무의 개수보다 커야한다는 점을 이용해서 경우를 나누어서 답을 구하면 된다.

단, $cnt_2 = \frac{S - cnt_1}{2}$이므로 $S - cnt_1$이 짝수라는 것을 주의해야한다.
...
당연히 매 순간마다 해당 원소에 지금까지 얼마나 더했는지 계산하면 $O(n^2)$이므로 시간 내에 통과할 수 없다.
...
이때 $s_i$는 $s_i = \sum{j = i}^{i + k - 1} c_j$이다.

이를 통해 지금까지 $a_i$에 더한 총합을 $O(1)$에 구할 수 있으므로, 전체를 $O(n)$으로 처리할 수 있다.
