...
위 식을 증명하기 위해서는 **같은 연결요소에 동일한 수가 들어있으면 최적의 방법이 아님**을 보여야한다.

연결 요소 $i_1 \to i_2 \to \dots \to i_x$에 대해 $a_{i_1} = a_{i_y}$인 $y$가 존재한다고 가정하자.

그렇다면 위의 연결요소는 $i_1 \to \dots \to i_{y-1}$와 $i_y \to \dots \to i_x$로 나눌 수 있다.

따라서 최적해에는 같은 연결요소에 같은 수가 있으면 안된다.

가장 많이 나타나는 수를 모두 다른 연결요소에 속하게 하면 $cc \geq \max(cnt)$이므로 $\max(s_a) \leq n - \max(cnt)$가 성립한다.
...
위 사실을 이용하면 $cc \leq \max(cnt)$이므로, $\max(s_a) = n - \max(cnt)$를 얻을 수 있다.

> $cnt_i = \max(cnt)$를 만족하는 정수 $i$를 하나 선택한 후, $i$에서 시작해서 다음 규칙에 따라 swap할 원소를 선택해나간다.
> 
> - 현재 원소보다 크고 swap이 가능한 원소 중 가장 작은 원소와 swap한다.
> - 단, 최댓값인 원소는 swap이 가능한 원소 중 최솟값인 원소와 swap한다.
> 
> 각 원소는 하나의 연결요소에만 속해야하므로 swap한 원소를 다시 swap하지 않아야한다.

예를 들어, $n = 5$, $a = \[1, 3, 4, 4, 3\]$이면 아래와 같은 과정을 거친다.

집합 $S$를 swap이 가능한 원소들의 집합이라 하고, $i = 3$으로 선택하면,

1. 초기 상태: $S = \\{ 1, 4, 4 \\}$
2. swap(3, 4), swap(4, 1): $3 \to 4 \to 1$, $S = \\{ 4 \\}$ 
4. swap(3, 4): $3 \to 4$, end.

즉, 이렇게 swap하면 $i$로 시작하는 연결요소 $cnt_i$개를 만들 수 있고, 

$\max(s_a) = n - \max(cnt)$라는 것 까지 증명하자.
