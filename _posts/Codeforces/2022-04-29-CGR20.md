...
또, $s_a(b)$를 수열 $a$에 대한 수열 $b$의 sadness라고 정의하자.
...
$cc = \max(cnt)$을 만족하는 swap 방법은 아래와 같다.

> 수열 $a$의 모든 원소를 해당 원소보다 크고 swap이 가능한 원소 중 가장 작은 것과 swap한다.
> 
> 최댓값인 원소는 swap이 가능한 원소 중 최솟값인 원소와 swap한다.
> 
> 각 원소는 하나의 연결요소에만 속해야하므로 swap한 원소를 다시 swap하지 않는다.

예를 들어, $n = 5$, $a = \[1, 3, 4, 4, 3\]$이면 아래와 같은 과정을 거친다.

집합 $S$를 swap이 가능한 원소들의 집합이라 하면,

1. 초기 상태: $S = \\{ 1, 3, 3, 4, 4 \\}$
2. $a_1= 1$, swap(1, 3): $S = \\{ 1, 3, 4, 4 \\}$
3. $a_2 = 3$, swap(3, 4): $S = \\{ 1, 3, 4 \\}$
4. $a_3 = 4$, swap(4, 1): $S = \\{ 3, 4 \\}$
5. $a_4 = 4$, swap(4, 3): $S = \\{ 4 \\}$
6. $a_5 = 3$, swap(3, 4): end
