...
최적으로 half-queen을 놓는 방법은 다음과 같다.
...
$n = 3m$에 대해 $k = m - 1$로 하면, $5$번 판에서 남은 두 개의 판은 $(m, 2m)$, $2m, m$이므로 $(m, m)$ 또는 $(2m, 2m)$ 위치에 half-queen을 배치하면 된다.

마지막으로, **이렇게 배치하는 것이 가장 최적인 이유는 그 어떠한 두 half-queen도 같은 행, 같은 열, 같은 대각선 위에 위치하지 않은 조건 하에서 얻은 최솟값이기 때문**이다.
