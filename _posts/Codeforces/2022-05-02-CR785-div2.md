...
기호 $\and$은 거듭제곱 또는 XOR 연산으로 사용되는 모호한 연산자이다.

아래와 같이 $\and$로 이루어진 모호한 식

\begin{aligned}
E = A_1 \and A_2 \and \dots \and A_n
\end{aligned}

에서 $\and$를 거듭제곱 또는 XOR 연산으로 바꿈으로서 모호하지 않은 식으로 바꿀 수 있다.
...
수열 $A$의 각 원소는 $A_i = 2^{B_i}$이며, 모호한 식 $E = A_1 \and A_2 \and \dots \and A_n$를 생각하자.
...
정수 $l, r$($1 \leq l < r \leq n$)에 대하여 $\[A_l \and A_{l+1} \and \dots \and A_r\]$을 생각하자.

위에 포함된 $\and$는 모두 거듭제곱으로 바꾸고, $A_l$ 왼쪽에 있는 연산자와 $A_r$ 오른쪽에 있는 연산자를 XOR로 바꾸자.

구하려는 값은 가능한 모든 $E'$들에 대해 XOR 연산를 수행한 것이므로, 위의 값은 그대로 결과에 영향을 주게 된다.

즉, 모든 $l, r$에 대하여 가능한 모든 $E'$에 등장하는 $\[A_l \and A_{l+1} \and \dots \and A_r\]$의 개수를 구해 XOR 연산을 해주면 원하는 값을 얻을 수 있다.

이를 구하기 위해서는 다음 두 가지를 알아야한다.

1. $S(l, r)$ : $( \dots (( A_l ^ {A_{l+1}\} ) ^ {A_{l+2}\} ) \dots )^ A_r \pmod{MOD}$ 의 값
2. $C(l, r)$ : 가능한 모든 $E'$에 $\[A_l \and A_{l+1} \and \dots \and A_r\]$ 가 나타나는 횟수
...
이므로, $S(l, r)$은 결과값의 $B_l \cdot A_{l + 1} \cdot A_{l+2} \cdot \dots \cdot A_r$번째 bit를 뒤집는다.

모듈러 연산을 적용한 값을 구해야하므로 만약 $S(l, r) \geq 2^{2^{20}\}$ 이면 무시해도 된다.

$B_i \geq 1$, $A_i \leq 2$ 이므로 $r - l \leq 20$ 인 경우만 고려해도 된다.

**2.** $S(l, r)$을 얻기 위해 $r-l$개의 $\and$를 거듭제곱으로, $0$, $1$ or $2$개의 $\and$를 XOR 연산자로 사용하였다.

남은 연산자의 개수를 $m = (n - 1) - (r - 1) - 2$라고 할 때, 이 중 적어도 $q$개를 $XOR$로 바꾸는 경우의 수는 다음과 같다.
...
\equiv {m-1 \choose q-1} \pmod{2}
\end{aligned}

이 값은 Lucas Theorem을 이용해서 빠르게 구할 수 있다.
