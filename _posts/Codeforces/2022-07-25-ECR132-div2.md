...
트리 위에서 어떤 경로의 weight를 해당 경로 위에 있는 정점들의 값들에 대하여 XOR 연산을 한 것이라고 정의하자.

이때 '좋은 트리'란 그 어떤 경로도 weight가 $0$이 아닌 트리를 의미한다.

그렇다면 주어진 트리의 정점들의 값을 마음대로 바꿀 수 있을 때, 트리를 좋은 트리로 만들기 위해 바꾸어야 하는 값의 최소 개수를 구하여라.
