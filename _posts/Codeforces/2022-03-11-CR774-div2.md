...
주어지는 그래프가 트리이기 때문에 다음과 같이 구할 수 있다.

1. 임의의 정점을 선택해서 트리의 루트 노드로 둔다.
2. 루트부터 트리를 탐색하면서 good 정점과 아닌 정점이 번갈아 나오도록 한다.
3. 위 작업을 **루트가 good 정점일 때와 루트가 good 정점이 아닐 때**에 대해서 각각 수행한 뒤, 더 큰 것을 찾는다.
4. 앞서 말한대로 무게를 할당하고 답을 출력한다.

트리를 탐색하는데 $O(n)$이므로 전체 시간복잡도도 $O(n)$이 된다.
