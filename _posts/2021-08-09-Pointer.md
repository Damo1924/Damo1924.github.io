...
# 3. C-stlye string symbolic constants
...
```
만약 위 함수가 다른 지역 변수의 주소를 반환하는 함수라면, 함수가 종료됨에 따라서 지역 변수는 없어지고 주소값만 반환하게 된다. String literals는 static duration을 가지기 때문에 "Park"는 없어지지 않고 함수를 통해 반환된 주소값으로 접근할 수 있게 된다.
